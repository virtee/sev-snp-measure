...
def main() -> int:
    ...
    parser.add_argument('--measurement', metavar='VALUE', type=str, help='Guest launch measurement', default=None)
    ...
    if args.idkey == None or args.authorkey == None:
        parser.error("missing key files for id block")
    ld = base64.b64decode(args.measurement)
    measurement = snp_calc_id_block(ld,args.idkey,args.authorkey)
...
def sign_in_snp_format(priv_key: ec.EllipticCurvePrivateKey, data: bytes) -> bytes:
    signature = priv_key.sign(data, ec.ECDSA(hashes.SHA384()))
    r, s = utils.decode_dss_signature(signature)
    sig_r = r.to_bytes(0x48, byteorder="little")
    sig_s = s.to_bytes(0x48, byteorder="little")
    signature: List[bytes] = []
    signature.extend(sig_r)
    signature.extend(sig_s)
    reserved = bytearray(368)
    signature.extend(reserved)
